...
use constants::js5_archive::js5_archive::CONFIG_OBJ;
...
pub fn unpack_objs() {
    ...
    for i in 0..js5_index.groups.len() {
        for (j, _) in js5_index.groups.get(&(i as u32)).unwrap().files.iter() {
            ...
            obj.decode_type(&mut Packet::from(cache.read(CONFIG_OBJ as u8, i as u32, *j as u16, None).unwrap()), &mut opcode_order);
